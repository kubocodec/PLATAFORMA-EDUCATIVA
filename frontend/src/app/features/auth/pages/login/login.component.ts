    ...
    <div class="login-container flex align-items-center justify-content-center min-h-screen">
      ...
      <div class="glass-panel p-5 w-full md:w-4 lg:w-3 flex flex-column align-items-center">
        <div class="text-center mb-5">
          <i class="pi pi-graduation-cap text-6xl text-primary mb-3" style="background: linear-gradient(90deg, #2979ff, #00e5ff); -webkit-background-clip: text; -webkit-text-fill-color: transparent;"></i>
          <h1 class="text-3xl font-bold m-0">Bienvenido</h1>
          <p class="text-gray-400 mt-2">Plataforma de Capacitación Continua</p>
        </div>

        <form [formGroup]="loginForm" (ngSubmit)="onSubmit()" class="w-full flex flex-column gap-4">
          <div class="flex flex-column gap-2">
            <label htmlFor="email" class="font-medium text-sm text-gray-300">Correo Electrónico</label>
            <input pInputText id="email" type="email" formControlName="email" placeholder="usuario@empresa.com" class="w-full" />
          </div>

          <div class="flex flex-column gap-2">
            <label htmlFor="password" class="font-medium text-sm text-gray-300">Contraseña</label>
          ...
          <div class="flex align-items-center justify-content-between text-sm mt-2 mb-3">
            <a href="#" class="text-blue-400 no-underline hover:text-blue-300 transition-colors">¿Olvidaste tu contraseña?</a>
          </div>
          ...
          <div class="text-center text-sm text-gray-400">
            ¿No tienes cuenta? <a href="#" class="text-blue-400 no-underline hover:text-blue-300 font-medium">Regístrate aquí</a>
          </div>
        </form>
      </div>
