    ...
    <div class="login-container flex align-items-center justify-content-center min-h-screen">
      <div class="glass-panel p-5 w-full md:w-5 lg:w-4 flex flex-column align-items-center">
        <div class="text-center mb-5">
          <i class="pi pi-user-plus text-5xl text-primary mb-3" style="background: linear-gradient(90deg, #2979ff, #00e5ff); -webkit-background-clip: text; -webkit-text-fill-color: transparent;"></i>
          <h1 class="text-3xl font-bold m-0">Crear Cuenta</h1>
          <p class="text-gray-400 mt-2">Únete a la plataforma de capacitación</p>
        </div>

        <form [formGroup]="registerForm" (ngSubmit)="onSubmit()" class="w-full flex flex-column gap-4">
          <div class="flex flex-column gap-2">
            <label htmlFor="fullName" class="font-medium text-sm text-gray-300">Nombre Completo</label>
          ...
          <div class="text-center text-sm text-gray-400">
            ¿Ya tienes cuenta? <a href="/login" class="text-blue-400 no-underline hover:text-blue-300 font-medium">Inicia sesión</a>
          </div>
        </form>
      </div>
    </div>
